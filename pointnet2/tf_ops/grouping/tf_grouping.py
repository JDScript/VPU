...
if __name__ == '__main__':
    ...
    np.random.seed(100)
    pts = np.random.random((32, 512, 3)).astype('float32')
    # tmp1 = np.random.random((32, 512, 3)).astype('float32')
    # tmp2 = np.random.random((32, 128, 3)).astype('float32')
    with tf.variable_scope("debug", reuse=tf.AUTO_REUSE):
        ...
        nsample = 64
        if knn:
            _, idx = knn_point(nsample, points, points, unique=True, sort=True)
            pc_neighbors = tf.gather_nd(points, idx)
        else:
            idx, _ = query_ball_point(radius, nsample, points, points)
            # idx = tf.expand_dims(idx, axis=-1)
            pc_neighbors = group_point(points, idx)
    ...
    with tf.Session('') as sess:
        ...
        pc_neighbors, ret = sess.run((pc_neighbors, idx))
        print((time.time() - now))
